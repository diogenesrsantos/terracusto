...
export default async function FuelDispensesPage() {
  ...
  const [tanks, works, assets, people, dispenses] = await Promise.all([
  ...
  return <><PageHead title="Abastecimentos" subtitle="Registre a saída do tanque para equipamentos e obras." />
    ...
    <section className="card mt"><h2>Últimos abastecimentos</h2>{dispenses.length === 0 ? <Empty /> : <div className="table-wrap"><table><thead><tr><th>Data</th><th>Tanque</th><th>Combustível</th><th>Equipamento</th><th>Obra</th><th>Litros</th><th>Operador</th></tr></thead><tbody>{dispenses.map((d) => <tr key={d.id}><td>{d.date.toLocaleDateString("pt-BR", { timeZone: "UTC" })}</td><td>{d.tank.name}</td><td>{d.fuelType.name}</td><td>{d.asset.identifier}</td><td>{d.work.code}</td><td>{number(d.liters, 3)}</td><td>{d.person?.name || "—"}</td></tr>)}</tbody></table></div>}</section>
  </>;
}
