...
export default async function WorksPage() {
  ...
  return <><PageHead title="Obras" subtitle="Cada obra corresponde a um único centro de custo." />
    <section className="card"><h2>Nova obra</h2><form action={createWork} className="form-grid">
      <label className="field">Código<input name="code" required /></label><label className="field span-2">Nome da obra<input name="name" required /></label>
      <label className="field">Início<input name="startDate" type="date" /></label><label className="field span-2">Cliente<input name="client" required /></label>
      <label className="field">Descrição<input name="description" /></label><button className="btn">Cadastrar obra</button>
    </form></section>
