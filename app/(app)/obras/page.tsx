...
export default async function WorksPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const works = await db.work.findMany({ orderBy: [{ code: "asc" }, { id: "asc" }], skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE });
  ...
  return <><PageHead title="Obras" subtitle="Cada obra corresponde a um único centro de custo." />
    <WorksManager key={page} works={works.map((work) => ({
      id: work.id, code: work.code, name: work.name, client: work.client, description: work.description,
    ...
    }))} page={page} totalPages={totalPages} totalWorks={totalWorks} />
  </>;
}
