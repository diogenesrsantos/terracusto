...
type ReportMode = "summary" | "detail" | "equipment" | "account-summary" | "account-detail";
...
const reportModeLabels: Record<ReportMode, string> = {
  ...
  equipment: "Agrupado por equipamento",
};
...
const hoursLabel = (value: number) => {
  ...
  return `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")} h`;
};
...
export default async function CostCenterReportPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const mode: ReportMode = params.mode === "detail" || params.mode === "equipment" || params.mode === "account-summary" || params.mode === "account-detail" ? params.mode : "summary";
  ...
  const equipmentMap = new Map<string, { asset: NonNullable<typeof reportEntries[number]["asset"]>; entries: typeof reportEntries; hours: number }>();
  for (const entry of reportEntries) {
    if (!entry.asset || !entry.hours || Number(entry.hours) <= 0 || !entry.startAt || !entry.endAt) continue;
    const current = equipmentMap.get(entry.asset.id) || { asset: entry.asset, entries: [], hours: 0 };
    current.entries.push(entry);
    current.hours += Number(entry.hours);
    equipmentMap.set(entry.asset.id, current);
  }
  const equipmentReports = Array.from(equipmentMap.values()).sort((left, right) => left.asset.identifier.localeCompare(right.asset.identifier, "pt-BR", { numeric: true }));
  const equipmentTotalHours = equipmentReports.reduce((total, equipment) => total + equipment.hours, 0);
  ...
  return <>
    ...
    {selectedWork && selectedCompetences.length > 0 && <section className="report-sheet">
      ...
      {mode === "summary" ? <>
        ...
        <h2>Jornadas agrupadas por equipamento</h2>
        {equipmentReports.length === 0 ? <Empty>Nenhuma jornada de equipamento encontrada para as competências selecionadas.</Empty> : <>
          {equipmentReports.map(({ asset, entries, hours }) => <section className="equipment-report-group" key={asset.id}>
            <h3>Equipamento: {asset.identifier} — {asset.description}</h3>
            <div className="table-wrap"><table><thead><tr><th>Data</th><th>Períodos</th><th className="text-right">Horas trab.</th><th>Horímetro</th><th>Operador</th></tr></thead><tbody>{entries.map((entry) => {
              const periods = [`${timeInput(entry.startAt)} às ${timeInput(entry.endAt)}`, entry.secondStartAt && entry.secondEndAt ? `${timeInput(entry.secondStartAt)} às ${timeInput(entry.secondEndAt)}` : null].filter(Boolean).join(" — ");
              const meter = entry.meterStart !== null && entry.meterEnd !== null ? `${number(entry.meterStart)} — ${number(entry.meterEnd)}` : "—";
              return <tr key={entry.id}><td>{date(entry.date)}</td><td>{periods}</td><td className="text-right">{hoursLabel(Number(entry.hours))}</td><td>{meter}</td><td>{entry.person?.name || "—"}</td></tr>;
            })}</tbody><tfoot><tr><th colSpan={2}>Total de horas: {asset.identifier} — {asset.description}</th><th className="text-right">{hoursLabel(hours)}</th><th colSpan={2}>—</th></tr></tfoot></table></div>
          </section>)}
          <div className="equipment-report-total">Total geral: <strong>{hoursLabel(equipmentTotalHours)}</strong></div>
        </>}
      ...
      <p className="report-footer">Emitido em {date(new Date())} · Total de horas: {mode === "equipment" ? hoursLabel(equipmentTotalHours) : number(totalHours)}</p>
    </section>}
  </>;
}
