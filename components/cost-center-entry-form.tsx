...
export function CostCenterEntryForm({
  ...
  return <><section className="card"><h2>{editing ? "Alteração de lançamento" : "Novo lançamento"}</h2><form action={submitEntry} className="form-grid">
    ...
    <fieldset className="form-grid span-4" disabled={!selectedWork || selectedWork.blocked}>
      ...
      <label className="field span-2">Histórico<input name="history" value={history} onChange={(event) => setHistory(event.target.value)} required /></label><label className="field">Documento<input name="document" value={document} onChange={(event) => setDocument(event.target.value)} /></label>
